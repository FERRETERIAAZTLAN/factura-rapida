...
const code=fs.readFileSync("solrak-caja-cortes-v0184.js","utf8");
const apiCode=fs.readFileSync("supabase/functions/cash-dashboard-api/index.ts","utf8");
const assert=(condition,message)=>{if(!condition)throw new Error(message)};
assert(!/cfdi-api|finkok|completeSale/i.test(code+apiCode),"Caja v0.1.84 invadió CFDI/cobro");
...
assert(/movement_type==="payment"/.test(apiCode)||/\.eq\("movement_type","payment"\)/.test(apiCode),"Falta desglose de abonos");
...
assert(window.SOLRAKCajaCortesV0184?.version==="0.1.84","No montó v0.1.84");
...
assert(doc.getElementById("solrakCash84Cards").textContent.includes("$650.00"),"No muestra efectivo teórico");
assert(doc.getElementById("solrakCash84Payments").textContent.includes("$500.00"),"No muestra efectivo de ventas");
assert(doc.getElementById("solrakCash84Payments").textContent.includes("$50.00"),"No muestra abonos en efectivo");
assert(doc.getElementById("solrakCash84Movements").textContent.includes("Fondo adicional")&&doc.getElementById("solrakCash84Movements").textContent.includes("Retiro"),"No muestra entradas/salidas");
doc.getElementById("solrakCash84Counted").value="645.50";doc.getElementById("solrakCash84CloseTurn").click();await new Promise(r=>setTimeout(r,40));
...
window.close();
console.log("SOLRAK_CAJA_CORTES_V0184_OK expected=650 payments=true credits=true movements=true closeDelegates=true");