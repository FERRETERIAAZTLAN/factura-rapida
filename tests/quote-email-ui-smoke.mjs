...
<input id="quoteCustomerEmail" value="">
...
let sent=false,sendCalls=0,listCalls=0,promptCalls=0,lastNotice='';
window.prompt=()=>{promptCalls++;return 'otro@example.com'};
...
window.eval('var fetch=(...args)=>window.__fetch(...args);');
...
if(listCalls<1)throw new Error('El módulo no consultó listQuotes');
...
if(promptCalls!==0)throw new Error(`Se volvió a pedir el correo aunque ya estaba guardado: promptCalls=${promptCalls}`);
...
console.log(`QUOTE_EMAIL_UI_SMOKE_OK listCalls=${listCalls} sendCalls=${sendCalls} promptCalls=${promptCalls}`);
