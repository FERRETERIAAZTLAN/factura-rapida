...
window.XLSX = {
  ...
  writeFile:()=>{}, read:()=>({ SheetNames:[], Sheets:{} })
};
window.fetch = async (url, options = {}) => {
  ...
  if (u.includes('/factura-api')) {
    if (action === 'login') {
      ...
      return response({ token:'test-session-token', expiresAt:new Date(Date.now()+3600000).toISOString(), user:{id:'u1',name:'Admin Prueba',role:'admin'}, business:{id:'b1',name:'Negocio Prueba',code:'AZTLAN'} });
    }
    ...
    if (action === 'me') return response({ user:{id:'u1',name:'Admin Prueba',role:'admin'}, business:{id:'b1',name:'Negocio Prueba',code:'AZTLAN'} });
...
try { window.eval(coreMatch[1]); }
catch (e) { throw new Error('Script principal inválido en runtime: '+(e?.stack||e)); }
try { window.eval(cleanMatch[1]); }
catch (e) { throw new Error('Script login-clean inválido en runtime: '+(e?.stack||e)); }
...
assert.equal(typeof form?.onsubmit, 'function', 'El handler de login debe quedar registrado');

// Reproduce el bloqueo histórico y comprueba recuperación.
...
business.value='AZTLAN'; user.value='admin'; pin.value='1234';
assert.equal(business.value,'AZTLAN'); assert.equal(user.value,'admin'); assert.equal(pin.value,'1234');

assert.equal(window.__frLoginPending,false,'El login debe iniciar sin solicitud pendiente');
form.dispatchEvent(new window.Event('submit',{bubbles:true,cancelable:true}));
await new Promise(r=>setTimeout(r,5));
assert.equal(loginCalls,1,'El primer submit debe iniciar exactamente una solicitud');
assert.equal(window.__frLoginPending,true,'La guardia debe activarse mientras inicia sesión');
form.dispatchEvent(new window.Event('submit',{bubbles:true,cancelable:true}));
...
console.log('SMOKE OK: runtime exacto del paquete, login interactivo bajo loading, doble-submit protegido, sesión y navegación OK.');
dom.window.close();
