...
for (const marker of [
  ...
  'diálogo/controlador de Windows',
...
assert(!/print_to_printer|silent_print/i.test(js + rust), "v0.1.91 no debe prometer impresión directa/silenciosa sin un comando nativo verificado");
...
assert(window.SOLRAKPrinter?.directPrint === false, "La impresora preferida debe declarar explícitamente directPrint=false mientras Windows muestra su diálogo");
