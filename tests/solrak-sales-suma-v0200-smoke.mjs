...
assert(document.documentElement.dataset.solrakSales200 === '1', 'No activó el layout estructural v0.2.00');
assert(workspace, 'No se creó workspace estructural');
assert(workspace.querySelector(':scope > .frPosTop'), 'Agregar Inventario no está dentro del workspace nuevo');
assert(workspace.querySelector('.s200Left > #realSearchCard') === originalSearchCard, 'Se clonó/reemplazó la búsqueda real');
assert(workspace.querySelector('.s200Left > #realCartCard') === originalCartCard, 'Se clonó/reemplazó el carrito real');
assert(workspace.querySelector('.s200Right#realSummary') === originalSummary, 'Se clonó/reemplazó el resumen real');
assert(document.getElementById('posSearch') === originalSearch, 'Se reemplazó el input real del POS');
assert(document.getElementById('fielFinishSale') === originalFinish, 'Se reemplazó FINALIZAR VENTA');
assert(document.querySelector('[data-fiel-pos-tool="discount"]') === originalDiscount, 'Se reemplazó acción real de descuento');

originalFinish.click();
originalDiscount.click();
assert(finishClicks === 1, 'Se perdió el handler real de finalizar venta');
assert(discountClicks === 1, 'Se perdió el handler real de descuento');

const oldGrid = document.querySelector('[data-s200-old-grid="1"]');
assert(oldGrid?.hidden === true, 'La estructura vieja sigue visible debajo de la nueva');
assert(document.getElementById('solrakSalesPhotoV0199Style')?.media === 'not all', 'La capa visual v0.1.99 sigue compitiendo');
assert(document.getElementById('solrakSalesExactV0198Style')?.media === 'not all', 'La capa visual v0.1.98 sigue compitiendo');
...
assert(document.querySelectorAll('.frPosCartHead span').length === 7, 'La tabla no conserva siete columnas');
for (const label of ['Código','Producto','Cantidad','Precio','Importe','Mayoreo','Descuento']) assert(document.querySelector('.frPosCartHead')?.textContent.includes(label), `Falta columna ${label}`);
...
for (const marker of [
  ...
  'grid-template-columns:minmax(0,1fr) var(--s200-right)',
  '.s200SearchCard',
  '.s200CartCard',
  '.s200Right',
  'background:linear-gradient(90deg,var(--s200-orange),var(--s200-orange2))',
  'background:linear-gradient(90deg,#ffad00,#ffc21a)',
  'font:400 clamp(42px,4vw,58px)/1 Georgia'
]) assert(style.includes(marker), `Falta contrato visual estructural: ${marker}`);
