...
assert(/\.eq\("cash_session_id",session\.id\)/.test(apiCode),"El resumen no está delimitado por sesión técnica");
...
for(const legacy of ["posCloseCash","posCountedCash","solrakCash85CloseTurn","function closeTurn"]){
  assert(!code.includes(legacy),`Caja conserva cierre manual legado: ${legacy}`);
}
assert(code.includes("Corte automático por franja horaria"),"Caja no explica el corte automático");

const dom=new JSDOM(`<!doctype html><html><head></head><body><button data-fiel-action="cash-cut">Corte de Caja</button></body></html>`,{runScripts:"dangerously",pretendToBeVisual:true,url:"https://example.test"});
const {window}=dom;
...
assert(!doc.getElementById("solrakCash85Counted"),"Reapareció captura manual de efectivo contado");
assert(!doc.getElementById("solrakCash85CloseTurn"),"Reapareció cierre manual de turno");
assert(doc.querySelector(".solrakCash85Foot")?.textContent.includes("no abre ni cierra turnos manualmente"),"No comunica arquitectura automática al cajero");
window.close();
console.log("SOLRAK_CAJA_CORTES_V0185_OK expected=650 payments=true credits=true movements=true automaticShift=true");
