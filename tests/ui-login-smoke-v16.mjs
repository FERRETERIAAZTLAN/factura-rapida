...
if (typeof form?.onsubmit !== 'function') {
  const detail = jsErrors.map(e => e?.message || String(e)).join(' | ');
  throw new Error('El handler de login no quedó registrado en runtime. Errores: '+detail);
}
...
assert.equal(loginCalls, 1, 'El primer submit debe iniciar exactamente una solicitud de login');
assert.equal(window.__frLoginPending, true, 'La guardia de login debe activarse durante la solicitud');
form.dispatchEvent(new window.Event('submit', { bubbles: true, cancelable: true }));
