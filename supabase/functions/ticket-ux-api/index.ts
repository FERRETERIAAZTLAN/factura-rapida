...
Deno.serve(async(req:Request)=>{
  ...
  try{
    if(action==="health")return json({ok:true,service:"ticket-ux-api",version:1,exactFolioBarcode:true,businessSetting:true,cfdiUntouched:true});
    ...
    if(action==="getSettings"){
      ...
      const result=await db.from("pos_settings").select("ticket_barcode_enabled,updated_at").eq("business_id",ctx.businessId).single();
      if(result.error)throw result.error;
      return json({ok:true,ticketBarcodeEnabled:result.data.ticket_barcode_enabled!==false,updatedAt:result.data.updated_at});
    }
    if(action==="setBarcode"){
      if(ctx.user.role!=="admin")return json({error:"Solo el administrador puede cambiar la configuración del ticket"},403);
      if(typeof body?.enabled!=="boolean")return json({error:"Estado inválido"},400);
      const result=await db.rpc("solrak_set_ticket_barcode",{p_business_id:ctx.businessId,p_user_id:ctx.user.id,p_enabled:body.enabled});
      if(result.error)throw result.error;
      return json({ok:true,ticketBarcodeEnabled:result.data?.ticket_barcode_enabled!==false,previous:result.data?.previous});
    }
    return json({error:"Acción desconocida"},400);
  }catch(error:any){console.error("ticket-ux-api",error);return json({error:"Error del servidor",detail:String(error?.message||error).slice(0,350)},500)}
});
