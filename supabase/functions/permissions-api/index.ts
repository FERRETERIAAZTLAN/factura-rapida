...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(action==="myPermissions"){
      return json({ok:true,user:{id:ctx.user.id,name:ctx.user.name,username:ctx.user.username,role:ctx.user.role},permissions:await permissionsFor(ctx.businessId,ctx.user)});
    }
    ...
    if(action==="listUsersPermissions"){
      ...
      const users=await db.from("app_users").select("id,name,username,role,active").eq("business_id",ctx.businessId).order("name");
      if(users.error)throw users.error;
      const rows=await db.from("pos_user_permissions").select("user_id,allow_discounts,allow_price_changes,allow_wholesale,allow_inventory_entry,updated_at").eq("business_id",ctx.businessId);
      if(rows.error)throw rows.error;
      const map=new Map((rows.data||[]).map((row:any)=>[row.user_id,row]));
      return json({ok:true,users:(users.data||[]).map((user:any)=>({
        ...user,
        permissions:user.role==="admin"?{...allAllowed}:{...defaults,...(map.get(user.id)||{})}
      }))});
    }
    ...
    if(action==="saveUserPermissions"){
      ...
      const target=(await db.from("app_users").select("id,role,active").eq("id",userId).eq("business_id",ctx.businessId).maybeSingle()).data;
      ...
      const payload={
        ...
        user_id:userId,
        allow_discounts:input.allow_discounts===true,
        allow_price_changes:input.allow_price_changes===true,
        allow_wholesale:input.allow_wholesale===true,
        allow_inventory_entry:input.allow_inventory_entry===true,
        updated_by:ctx.user.id,
        updated_at:new Date().toISOString()
      };
      const saved=await db.from("pos_user_permissions").upsert(payload,{onConflict:"business_id,user_id"}).select("allow_discounts,allow_price_changes,allow_wholesale,allow_inventory_entry").single();
