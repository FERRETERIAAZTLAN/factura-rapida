...
.solrakCash85Foot{display:grid;grid-template-columns:minmax(260px,1fr) auto auto;gap:7px;align-items:center;background:#fff;border:1px solid #d5dbe0;padding:8px}.solrakCash85Foot button{height:35px;border:1px solid #c8d0d6;border-radius:3px;background:#fff;padding:0 12px;font-weight:800;cursor:pointer}.solrakCash85AutoInfo{font-size:10px;color:#5f6d78}.solrakCash85AutoInfo strong{display:block;color:#2d3942;font-size:11px;margin-bottom:2px}.solrakCash85Empty{padding:25px;text-align:center;color:#76828b;font-size:11px}
@media(max-width:980px){.solrakCash85Cards{grid-template-columns:repeat(3,1fr)}.solrakCash85Grid{grid-template-columns:1fr}.solrakCash85Foot{grid-template-columns:1fr auto auto}}
...
`;
  ...
  function ensureDialog(){
    ...
    dialog.innerHTML=`<div class="solrakCash85Head"><div><strong>Caja y Corte</strong><small id="solrakCash85Status">Corte automático</small></div><button type="button" id="solrakCash85Close">×</button></div><div class="solrakCash85Body"><div id="solrakCash85Meta" class="solrakCash85Meta"></div><div id="solrakCash85Cards" class="solrakCash85Cards"></div><div class="solrakCash85Grid"><section class="solrakCash85Panel"><div class="solrakCash85PanelHead"><span>Formas de pago</span><span id="solrakCash85TicketCount"></span></div><div id="solrakCash85Payments" class="solrakCash85Pay"></div></section><section class="solrakCash85Panel"><div class="solrakCash85PanelHead"><span>Entradas y salidas de efectivo</span><span id="solrakCash85MovementCount"></span></div><div id="solrakCash85Movements" class="solrakCash85TableWrap"></div></section></div><div class="solrakCash85Foot"><div class="solrakCash85AutoInfo"><strong>Corte automático por franja horaria</strong>El cajero no abre ni cierra turnos manualmente. SOLRAK calcula el corte usando la franja vigente y conserva el historial.</div><button id="solrakCash85Csv" type="button">Exportar CSV</button><button id="solrakCash85Print" type="button">Imprimir</button></div></div>`;
  ...
  function render(data){
    ...
    byId("solrakCash85Status").textContent="Corte automático por horario";
    byId("solrakCash85Meta").innerHTML=`<span>Caja: <b>${esc(s.register_name)}</b></span><span>Inicio técnico: <b>${esc(date(s.opened_at))}</b></span><span>Usuario: <b>${esc(s.opened_by_name)}</b></span>${s.closed_at?`<span>Fin técnico: <b>${esc(date(s.closed_at))}</b></span>`:""}`;
  ...
  async function openDashboard(){
    ...
    try{render(await api("preview",{cashSessionId:active.id}))}catch(error){byId("solrakCash85Cards").innerHTML=`<div class="solrakCash85Empty">Error al cargar: ${esc(error.message)}</div>`;notice(error.message,true)}
  }
  ...
  function exportCsv(){
    ...
    lines.push(["SOLRAK - CORTE DE CAJA"],["Caja",s.register_name],["Inicio técnico",date(s.opened_at)],["Usuario",s.opened_by_name],[]);
