(() => {
  ...
  function anonKey() {
    try { return ANON_KEY || window.ANON_KEY || ""; } catch { return window.ANON_KEY || ""; }
  }

  function isAdmin() {
    return currentSession()?.user?.role === "admin" || state.user?.role === "admin";
  }
  ...
  function employeeTerminology() {
    ...
    const seller = role?.querySelector('option[value="seller"]');
    if (seller) seller.textContent = "Empleado · acceso según permisos";
    document.querySelectorAll("#userList .badge").forEach((badge) => {
      if (badge.textContent.trim() === "Vendedor") badge.textContent = "Empleado";
    });
    const userHelp = document.querySelector("#tab-usuarios .callout");
    if (userHelp) userHelp.innerHTML = "<strong>Empleado:</strong> accede únicamente a las funciones autorizadas. <strong>Administrador:</strong> tiene acceso completo y configura los permisos de cada empleado.";
  }
  ...
  function ensureManager() {
    ...
    if (!card) {
      ...
      card.id = "solrakPermissionsManager";
    ...
    card.innerHTML = `
      <div class="card-head"><div><h2>Permisos de empleados</h2><p class="muted small">Los usuarios se desactivan; no se eliminan. Los administradores conservan acceso completo.</p></div><button id="solrakPermissionsReload" class="secondary compact" type="button">Actualizar</button></div>
      <div class="table-wrap"><table><thead><tr><th>Usuario</th><th>Rol</th><th>Estado</th><th>Permisos</th><th></th></tr></thead><tbody>
      ${employees.length ? employees.map((user) => `<tr data-solrak-user-row="${esc(user.id)}"><td><strong>${esc(user.name)}</strong><div class="muted small">@${esc(user.username)}</div></td><td>${user.role === "admin" ? "Administrador" : "Empleado"}</td><td>${user.active === false ? "Inactivo" : "Activo"}</td><td><div class="solrakPermGrid">${Object.keys(LABELS).map((key) => permissionSwitch(user, key)).join("")}</div></td><td>${user.role === "admin" ? '<span class="badge admin">Acceso total</span>' : `<button class="primary compact" type="button" data-solrak-save-perms="${esc(user.id)}">Guardar permisos</button>`}</td></tr>`).join("") : '<tr><td colspan="5" class="empty">No hay usuarios para configurar.</td></tr>'}
      </tbody></table></div>`;

    if (!byId("solrakPermissionsStyle")) {
    ...
    byId("solrakPermissionsReload").onclick = () => refresh(true);
    card.querySelectorAll("[data-solrak-save-perms]").forEach((button) => {
      button.onclick = async () => {
        const userId = button.dataset.solrakSavePerms;
        const permissions = {};
        card.querySelectorAll(`[data-solrak-perm-user="${CSS.escape(userId)}"]`).forEach((input) => {
          permissions[input.dataset.solrakPermKey] = input.checked === true;
        });
        button.disabled = true;
        try {
          await api("saveUserPermissions", { userId, permissions });
          notice("Permisos del empleado guardados.");
          await refresh(true);
        } catch (error) {
  ...
  function scheduleRefresh() {
    ...
    refreshTimer = setTimeout(() => {
      ...
      refresh(isAdmin()).catch((error) => console.warn("SOLRAK permissions", error));
    }, 60);
  }
  ...
  function boot() {
    ...
    observer = new MutationObserver(() => {
      applyPermissionUi();
      if (isAdmin() && state.users.length) ensureManager();
    });
