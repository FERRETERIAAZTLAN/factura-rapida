(() => {
  "use strict";
  const POS_API =
    "https://jojzhohqrshsjmlirkqz.supabase.co/functions/v1/pos-api";
  const SUPPLIER_API =
  ...
  function apiUrl(url, action, payload = {}) {
    ...
    return fetch(url, {
      ...
      if (!r.ok) {
        const e = new Error(d.error || d.detail || "No se pudo continuar");
        e.data = d;
        throw e;
      }
      return d;
  ...
  const supplierApi = (action, payload = {}) =>
    apiUrl(SUPPLIER_API, action, payload);
  function currentTicket() {
    return posTickets.find((t) => t.id === activeTicketId) || posTickets[0];
  }
  function ticketTotals(ticket) {
    return (ticket?.cart || []).reduce(
      (a, i) => {
        const x = lineAmounts(i);
        a.base += x.base;
        a.tax += x.tax;
        a.total += x.total;
        return a;
  ...
  function syncTicket() {
    ...
    if (!t) return;
    t.cart = posCart;
    t.clientId = byId("posClient")?.value || t.clientId || "";
  }
  function injectStyle() {
    if (byId("frPosStyle")) return;
...
`;
  ...
  function injectUi() {
    if (byId("tab-pos")) return;
    injectStyle();
    const nav = document.querySelector(".nav");
    if (!nav) return;
    const posBtn = document.createElement("button");
    posBtn.dataset.tab = "pos";
    posBtn.id = "posTabBtn";
    posBtn.innerHTML =
      'Punto de venta <span class="count" id="posSaleCount">0</span>';
    posBtn.onclick = () => {
      switchTab("pos");
    ...
    supBtn.dataset.tab = "proveedores";
    supBtn.id = "supplierTabBtn";
    supBtn.innerHTML =
      'Proveedores <span class="count" id="supplierCount">0</span>';
    supBtn.onclick = () => switchTab("proveedores");
    nav.insertBefore(supBtn, posBtn.nextSibling);
...
<div class="frPosGrid"><div class="stack"><article class="card"><div class="card-head"><h2>Buscar producto</h2><span class="muted small" id="posProductMeta">0 disponibles</span></div><div class="frPosSearch"><input id="posSearch" class="field" autocomplete="off" placeholder="Escanea código o escribe producto…"><div class="frPosHint">Enter agrega la coincidencia exacta o el primer resultado.</div></div><div id="posResults" class="frPosResults"></div></article><article class="card frPosCartCard"><div class="card-head"><h2 id="posCartTitle">Ticket #1</h2><button id="posClear" class="ghost compact" type="button">Limpiar ticket</button></div><label>Cliente (opcional)<select id="posClient" class="field"><option value="">Público general</option></select></label><div class="frPosCartHead"><span>Código</span><span>Producto</span><span>Cantidad</span><span>Precio</span><span>Importe</span></div><div id="posCart" class="frPosCart"></div></article><article class="card"><div class="card-head"><h2>Ventas recientes</h2><button id="posRefresh" class="secondary compact" type="button">Actualizar</button></div><div id="posRecent" class="frRecent"></div></article></div><aside class="card summary"><div class="card-head"><h2>Venta actual</h2><span id="posTicketBadge" class="badge">Ticket #1</span></div><div id="posProductPreview" class="frPreview"><div class="frPreviewEmpty">Selecciona o agrega un producto para ver su imagen.</div></div><div class="frPosTotals"><div><span>Subtotal</span><strong id="posSubtotal">$0.00</strong></div><div><span>IVA</span><strong id="posIva">$0.00</strong></div><div class="frPosGrand"><span>Total</span><strong id="posTotal">$0.00</strong></div></div><button id="posCharge" class="primary full" type="button">Abrir caja para cobrar</button><div id="posReceipt"></div></aside></div>`;
    ...
    cutDlg.className = "frPosDialog";
    cutDlg.innerHTML = `<div class="frPosDialogInner frCutReport"><div class="card-head"><h2>Corte de caja</h2><button class="ghost" type="button" data-pos-close="posCutDialog">×</button></div><div id="posCutContent"></div><button class="primary full" type="button" data-pos-close="posCutDialog">Cerrar</button></div>`;
    document.body.appendChild(cutDlg);
    document
      .querySelectorAll("[data-pos-close]")
      .forEach((b) => (b.onclick = () => byId(b.dataset.posClose)?.close()));
    bindUi();
    renderTickets();
  }
  function lineAmounts(i) {
    const gross = Number(i.price || 0) * Number(i.qty || 0),
      rate = Number(i.iva ?? 16) / 100;
    if (i.price_includes_tax !== false) {
      const base = rate ? gross / (1 + rate) : gross;
      return { base, tax: gross - base, total: gross };
    }
    const tax = gross * rate;
    return { base: gross, tax, total: gross + tax };
  }
  function totals() {
    return ticketTotals({ cart: posCart });
  }
  function renderClients() {
    const el = byId("posClient");
    if (!el) return;
    const desired = currentTicket()?.clientId || el.value;
    el.innerHTML =
      '<option value="">Público general</option>' +
      clients
        .map(
          (c) =>
            `<option value="${c.id}">${htmlEsc(c.name)} · ${htmlEsc(c.rfc)}</option>`,
        )
        .join("");
  ...
  function productResults() {
    const q = (byId("posSearch")?.value || "").trim().toLowerCase();
    if (!q) return [];
    return products
  ...
  function renderProductPreview(p) {
    ...
    if (!p) {
      box.innerHTML =
        '<div class="frPreviewEmpty">Selecciona o agrega un producto para ver su imagen.</div>';
      return;
    }
  ...
  function renderResults() {
    ...
    if (!box) return;
    const query = (byId("posSearch")?.value || "").trim(),
      rows = productResults();
    byId("posProductMeta").textContent =
      `${products.filter((p) => p.active !== false).length} disponibles`;
    ...
    box.querySelectorAll("[data-pos-product]").forEach((b) => {
      b.onmouseenter = () =>
        renderProductPreview(
          products.find((p) => p.id === b.dataset.posProduct),
        );
      b.onclick = () =>
        addProduct(products.find((p) => p.id === b.dataset.posProduct));
    });
    if (!selectedProductId && rows[0]) renderProductPreview(rows[0]);
  }
  function addProduct(p) {
    ...
    const line = posCart.find((x) => x.id === p.id);
    if (line) {
      if (Number(line.qty) + 1 > Number(p.stock)) {
  ...
  function renderCart() {
    ...
    if (!box) return;
    const tkt = currentTicket();
    if (byId("posCartTitle"))
      byId("posCartTitle").textContent = `Ticket #${tkt.id}`;
    if (byId("posTicketBadge"))
      byId("posTicketBadge").textContent = `Ticket #${tkt.id}`;
    ...
    box.querySelectorAll("[data-pos-line]").forEach(
      (row) =>
        (row.onclick = (e) => {
          if (e.target.closest("button,input")) return;
          renderProductPreview(
            posCart.find((x) => x.id === row.dataset.posLine),
          );
        }),
    );
    box.querySelectorAll("[data-pos-q]").forEach(
      (b) =>
        (b.onclick = () => {
          const i = posCart.find((x) => x.id === b.dataset.id);
          if (!i) return;
          if (b.dataset.posQ === "minus")
            i.qty = Math.max(0.001, Number(i.qty) - 1);
          if (b.dataset.posQ === "plus")
            i.qty = Math.min(Number(i.stock), Number(i.qty) + 1);
          if (b.dataset.posQ === "remove")
            posCart = posCart.filter((x) => x.id !== i.id);
          currentTicket().cart = posCart;
          renderCart();
          renderTickets();
        }),
    );
    box.querySelectorAll("[data-pos-qty]").forEach(
      (inp) =>
        (inp.onchange = () => {
          const i = posCart.find((x) => x.id === inp.dataset.posQty);
          if (!i) return;
          i.qty = Math.min(
            Number(i.stock),
            Math.max(0.001, Number(inp.value) || 1),
          );
          currentTicket().cart = posCart;
          renderCart();
          renderTickets();
    ...
    const t = totals();
    byId("posSubtotal").textContent = mx(t.base);
    byId("posIva").textContent = mx(t.tax);
    byId("posTotal").textContent = mx(t.total);
    updatePayment();
    updateChargeButton();
  }
  function renderTickets() {
    const box = byId("posTickets");
    if (!box) return;
    box.innerHTML = posTickets
      .map((t) => {
        const tt = ticketTotals(t),
          items = t.cart.reduce((a, x) => a + Number(x.qty || 0), 0);
        return `<div class="frTicket ${t.id === activeTicketId ? "active" : ""}" role="button" tabindex="0" data-ticket="${t.id}"><strong>Ticket #${t.id}</strong><span class="frTicketClose" title="Cerrar ticket" data-close-ticket="${t.id}">×</span><small>${items ? `${items} prod. · ${mx(tt.total)}` : "Vacío"}</small></div>`;
      })
      .join("");
    box.querySelectorAll("[data-ticket]").forEach((el) => {
      el.onclick = (e) => {
        if (e.target.closest("[data-close-ticket]")) return;
        activateTicket(Number(el.dataset.ticket));
      };
      el.onkeydown = (e) => {
    ...
    box.querySelectorAll("[data-close-ticket]").forEach(
      (el) =>
        (el.onclick = (e) => {
          e.stopPropagation();
          closeTicket(Number(el.dataset.closeTicket));
        }),
    );
    if (byId("posNewTicket"))
      byId("posNewTicket").disabled = posTickets.length >= MAX_TICKETS;
  }
  function activateTicket(id) {
    if (id === activeTicketId) return;
    syncTicket();
    const next = posTickets.find((t) => t.id === id);
    if (!next) return;
    activeTicketId = id;
    posCart = next.cart;
    selectedProductId = posCart.at(-1)?.id || null;
    renderClients();
    renderCart();
    renderTickets();
    renderProductPreview(posCart.at(-1) || null);
    byId("posReceipt").innerHTML = "";
    resetPayment();
    setTimeout(() => byId("posSearch")?.focus(), 20);
  }
  function newTicket() {
    if (posTickets.length >= MAX_TICKETS) {
      notice(`Puedes mantener hasta ${MAX_TICKETS} tickets abiertos.`, true);
      return;
    }
    syncTicket();
    const t = { id: ticketSeq++, cart: [], clientId: "" };
    posTickets.push(t);
    activeTicketId = t.id;
    posCart = t.cart;
  ...
  function closeTicket(id) {
    ...
    const idx = posTickets.indexOf(t);
    posTickets = posTickets.filter((x) => x.id !== id);
    if (activeTicketId === id) {
      const next = posTickets[Math.min(idx, posTickets.length - 1)];
  ...
  function resetPayment() {
    if (byId("posPayment")) byId("posPayment").value = "cash";
    if (byId("posTendered")) byId("posTendered").value = "";
    if (byId("posReference")) byId("posReference").value = "";
    if (byId("posSaleNote")) byId("posSaleNote").value = "";
    document.querySelectorAll("[data-pay-amount]").forEach((input) => {
      input.value =
        input.dataset.payAmount === "cash" ? totals().total.toFixed(2) : "0.00";
    });
    updatePayment();
  }
  function paymentDraft() {
    const reference = byId("posReference")?.value.trim() || "";
    return [...document.querySelectorAll("[data-pay-amount]")]
      .map((input) => ({
        method: input.dataset.payAmount,
      ...
      .map((payment) => {
        if (payment.method === "cash")
          payment.tendered =
            Math.round((Number(byId("posTendered")?.value) || 0) * 100) / 100;
        else if (reference) payment.reference = reference;
        return payment;
      });
  }
  function updatePayment() {
    const total = Math.round(totals().total * 100) / 100,
      ...
      cash = payments.find((payment) => payment.method === "cash"),
      tendered = Number(byId("posTendered")?.value) || 0,
    ...
    document.querySelectorAll("[data-pay-card]").forEach((card) => {
      card.classList.toggle(
        "active",
        (Number(card.querySelector("[data-pay-amount]")?.value) || 0) > 0,
      );
    });
    ...
    if (byId("posBalance"))
      byId("posBalance").textContent = mx(remaining || excess);
    if (byId("posBalanceLabel"))
  ...
  function updateChargeButton() {
    ...
    if (!posCart.length) {
      b.textContent = "Agrega productos";
      b.classList.add("frSaleDisabled");
      return;
    }
  ...
  function openPayment() {
    ...
    if (!posCart.length) {
      notice("Agrega productos antes de cobrar.", true);
      return;
    }
    syncTicket();
    const t = totals(),
      ticket = currentTicket();
    byId("posPayTicket").textContent = `Ticket #${ticket.id}`;
    byId("posPayItems").textContent =
      `${posCart.length} ${posCart.length === 1 ? "producto" : "productos"}`;
    byId("posPayTotal").textContent = mx(t.total);
    resetPayment();
    const dlg = byId("posPayDialog");
    if (dlg?.showModal) dlg.showModal();
    else dlg?.setAttribute("open", "");
  }
  function renderState() {
    const st = byId("posCashState"),
      open = posState.openSession;
    if (st) {
      st.classList.toggle("open", !!open);
      st.textContent = open
        ? "Caja abierta · " +
          new Date(open.opened_at).toLocaleTimeString("es-MX", {
            hour: "2-digit",
            minute: "2-digit",
          })
        : "Caja cerrada";
    ...
    if (regs)
      regs.innerHTML = (posState.registers || [])
        .map((r) => `<option value="${r.id}">${htmlEsc(r.name)}</option>`)
        .join("");
    byId("supplierCount").textContent = String(
      posState.supplierCount || suppliers.length || 0,
    );
    byId("posSaleCount").textContent = String(
      (posState.recentSales || []).length,
    );
    updateChargeButton();
  }
  function renderRecent() {
    const box = byId("posRecent");
    if (!box) return;
    const rows = posState.recentSales || [];
    box.innerHTML = rows.length
      ? rows
          .map(
            (s) =>
              `<div class="frRecentRow"><div><strong>Venta #${String(s.sale_number).padStart(6, "0")}</strong><small>${new Date(s.created_at).toLocaleString("es-MX")} · ${htmlEsc(s.customer_name || "Público general")}</small></div><strong>${mx(s.total)}</strong></div>`,
          )
          .join("")
      : '<div class="frPosEmpty">Aún no hay ventas registradas.</div>';
  }
  async function refreshPos() {
    ...
    try {
      ...
      console.warn("POS bootstrap", e);
    }
  }
  async function charge() {
    if (chargeLock || !posState.openSession || !posCart.length) return;
    const t = totals(),
      payments = paymentDraft(),
      paid = payments.reduce((sum, payment) => sum + payment.amount, 0),
      cash = payments.find((payment) => payment.method === "cash"),
      tendered = Number(byId("posTendered").value) || 0;
    if (Math.abs(paid - t.total) > 0.01) {
      notice(
    ...
    if (cash && tendered + 0.001 < cash.amount) {
      notice(
        "El efectivo recibido es menor que el monto asignado a efectivo.",
        true,
      );
      updatePayment();
      return;
    ...
    try {
      const r = await posApi("completeSale", {
        cashSessionId: posState.openSession.id,
        clientId: chargedTicket.clientId || null,
        items,
        payments,
        notes: byId("posSaleNote")?.value.trim() || null,
      });
      const change = cash ? Math.max(0, tendered - cash.amount) : 0;
      chargedTicket.cart = [];
      chargedTicket.clientId = "";
      posCart = chargedTicket.cart;
      byId("posPayDialog")?.close?.();
      resetPayment();
      byId("posReceipt").innerHTML =
      ...
      await refreshPos();
      renderTickets();
      notice(`Venta #${r.sale_number} registrada. Inventario actualizado.`);
    } catch (e) {
      notice(e.message, true);
    } finally {
      chargeLock = false;
      if (byId("posConfirmCharge")) byId("posConfirmCharge").disabled = false;
      busy(false);
    }
  }
  function cutRow(label, value, klass = "") {
    return `<div class="frCutRow"><span>${htmlEsc(label)}</span><strong class="${klass}">${mx(value)}</strong></div>`;
  }
  function showCut(sessionData) {
    const box = byId("posCutContent");
    if (!box) return;
    const value = (...keys) => {
        for (const k of keys)
          if (Number.isFinite(Number(sessionData?.[k])))
            return Number(sessionData[k]);
        return null;
      },
      rows = [];
    const opening = value("opening_amount", "openingAmount"),
      cash = value("cash_sales", "cashSales"),
      card = value("card_sales", "cardSales"),
      transfer = value("transfer_sales", "transferSales"),
      credit = value("credit_sales", "creditSales"),
      other = value("other_sales", "otherSales"),
      refunds = value("refunds_total", "refundsTotal"),
      cancellations = value("cancellations_total", "cancellationsTotal"),
      expected = value("expected_cash", "expectedCash"),
      counted = value("counted_cash", "countedCash"),
      difference = value("difference");
    if (opening !== null) rows.push(cutRow("Fondo inicial", opening));
    if (cash !== null) rows.push(cutRow("Ventas en efectivo", cash, "good"));
    if (card !== null) rows.push(cutRow("Ventas con tarjeta", card, "good"));
    if (transfer !== null)
      rows.push(cutRow("Ventas por transferencia", transfer, "good"));
    if (credit !== null) rows.push(cutRow("Ventas a crédito", credit, "good"));
    if (other !== null) rows.push(cutRow("Otros métodos", other, "good"));
    if (refunds !== null)
      rows.push(cutRow("Devoluciones", -Math.abs(refunds), "bad"));
    if (cancellations !== null)
      rows.push(cutRow("Cancelaciones", -Math.abs(cancellations), "bad"));
    if (expected !== null)
      rows.push(cutRow("Efectivo esperado", expected, "good"));
    if (counted !== null) rows.push(cutRow("Efectivo contado", counted));
    if (difference !== null)
      rows.push(
        cutRow("Diferencia", difference, difference < 0 ? "bad" : "good"),
      );
    const opened = sessionData?.opened_at || sessionData?.openedAt,
      closed = sessionData?.closed_at || sessionData?.closedAt;
    box.innerHTML = `<h2>Corte de Caja</h2><div><strong>Periodo:</strong> ${opened ? new Date(opened).toLocaleString("es-MX") : "—"} a ${closed ? new Date(closed).toLocaleString("es-MX") : new Date().toLocaleString("es-MX")}</div><div class="frCutGrid">${rows.join("")}</div><div class="frPosHint" style="margin-top:12px">Se muestran únicamente importes devueltos por el cierre productivo; SOLRAK no estima ni inventa movimientos.</div>`;
    const dlg = byId("posCutDialog");
    if (dlg?.showModal) dlg.showModal();
    else dlg?.setAttribute("open", "");
  }
  function resetSupplier() {
    editingSupplierId = null;
    byId("supplierForm")?.reset();
    if (byId("supCredit")) byId("supCredit").value = "0";
    byId("supplierFormTitle").textContent = "Registrar proveedor";
    byId("supplierCancel").classList.add("hidden");
  }
  function renderSuppliers() {
    const q = (byId("supplierSearch")?.value || "").trim().toLowerCase(),
    ...
    byId("supplierMeta").textContent = `${rows.length} registros`;
    byId("supplierCount").textContent = String(
      suppliers.filter((s) => s.active !== false).length,
    );
    byId("supplierList").innerHTML = rows.length
      ? rows
          .map(
            (s) =>
              `<button type="button" class="frSupplierRow" data-supplier="${s.id}"><span><strong>${htmlEsc(s.name)}</strong><span>${htmlEsc(s.rfc || "Sin RFC")} · ${htmlEsc(s.contact_name || s.phone || s.email || "Sin contacto")}</span></span><span class="badge ${s.active !== false ? "good" : "warn"}">${s.active !== false ? "Activo" : "Inactivo"}</span></button>`,
          )
          .join("")
      : '<div class="frPosEmpty">No hay proveedores.</div>';
    byId("supplierList")
      .querySelectorAll("[data-supplier]")
      .forEach(
        (b) =>
          (b.onclick = () =>
            editSupplier(suppliers.find((s) => s.id === b.dataset.supplier))),
      );
    const admin = isAdmin();
    byId("supplierForm").classList.toggle("hidden", !admin);
    byId("supplierReadOnly").classList.toggle("hidden", admin);
    byId("supplierNew").classList.toggle("hidden", !admin);
  }
  function editSupplier(s) {
    if (!s || !isAdmin()) return;
    editingSupplierId = s.id;
    byId("supName").value = s.name || "";
    byId("supLegal").value = s.legal_name || "";
    byId("supRfc").value = s.rfc || "";
    byId("supCredit").value = s.credit_days || 0;
    byId("supContact").value = s.contact_name || "";
    byId("supPhone").value = s.phone || "";
    byId("supWhatsapp").value = s.whatsapp || "";
    byId("supEmail").value = s.email || "";
    byId("supAddress").value = s.address || "";
    byId("supNotes").value = s.notes || "";
    byId("supplierFormTitle").textContent = "Editar proveedor";
    byId("supplierCancel").classList.remove("hidden");
  }
  async function refreshSuppliers() {
    if (!session?.token) return;
    try {
      const d = await supplierApi("listSuppliers", { activeOnly: false });
  ...
  function bindUi() {
    byId("posSearch").oninput = renderResults;
    byId("posSearch").onkeydown = (e) => {
      if (e.key !== "Enter") return;
      e.preventDefault();
      const q = e.target.value.trim().toLowerCase(),
        exact = products.find(
          (p) => p.active !== false && String(p.code || "").toLowerCase() === q,
        ),
        first = productResults()[0];
      addProduct(exact || first);
    };
    byId("posClient").onchange = () => {
    ...
    document.querySelectorAll("[data-pay-card]").forEach((card) => {
      card.onclick = (event) => {
        ...
        const input = card.querySelector("[data-pay-amount]");
        const paid = paymentDraft().reduce(
          (sum, payment) => sum + payment.amount,
          0,
        );
        if (input && !(Number(input.value) > 0))
          input.value = Math.max(0, totals().total - paid).toFixed(2);
        input?.focus();
        input?.select();
        updatePayment();
      };
    ...
    document.querySelectorAll("[data-pay-amount]").forEach((input) => {
      input.oninput = updatePayment;
      input.onfocus = () => {
        if (byId("posPayment"))
          byId("posPayment").value = input.dataset.payAmount;
      };
    });
    byId("posTendered").oninput = updatePayment;
    ...
    byId("supplierForm").onsubmit = async (e) => {
      e.preventDefault();
      busy(true);
      try {
        await supplierApi("saveSupplier", {
          id: editingSupplierId || undefined,
          name: byId("supName").value.trim(),
          legalName: byId("supLegal").value.trim(),
          rfc: byId("supRfc").value.trim(),
          creditDays: Number(byId("supCredit").value) || 0,
          contactName: byId("supContact").value.trim(),
          ...
          notes: byId("supNotes").value.trim(),
        });
        resetSupplier();
  ...
  try {
    ...
    loadAll = async function () {
      ...
      setTimeout(afterDataLoad, 0);
      return r;
    };
  } catch (e) {
    console.warn("POS load wrapper", e);
  }
  setTimeout(() => {
    if (session?.token) afterDataLoad();
  }, 900);
